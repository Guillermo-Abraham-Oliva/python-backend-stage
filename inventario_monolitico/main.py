...
BASE = Path(__file__).resolve().parent # folder donde está main.py
DATA = BASE / "data"                   # /Inventario_Monolitico/data  <- inventario & update
OUT = BASE / "salida"                  # /Inventario_Monolitico/salida <- catalogo & reporte

def cargar_json(ruta: Path) -> List[Dict[str, Any]]:
    try:
        with open(ruta, "r", encoding="utf-8") as f:
            return json.load(f)
        # alternativa para pequeños scripts y archivos pequeños (20%):
...
def a_int(x: Any) -> int: # Conversion a entero
    if isinstance(x, int):
        return x
...
def generar_reporte(catalogo_final: List[Dict[str, Any]], ids_baja: List[int]) -> Dict[str, Any]:
    ...
    top3_dicts = sorted(catalogo_final, key=lambda o: o["stock"], reverse=True) # creo una lista ordenada -de mayor a menor- por stock
    top3_lst = top3_dicts[:3] # creo otra lista con los 3 con mas stock
...
def guardar_resultados(ruta: Path, data: Any) -> None:
    try:
        ...
        with open(ruta, "w", encoding="utf-8") as f:
            ...
            # no va return porque no retorna nada, solo opera externamente
    except PermissionError:
...
def main() -> None:
    # mkdir -> crear directorio de salida y sus 'padres' si no existen (parents=True). 
    # exist_ok=True -> Si ya existe la carpeta, continuar ejecucion sin error ni detencion
    ...
    # NORMALIZAMOS (guardarmos en un listado de normalizados)
    ...
    guardar_resultados(OUT / "catalogo_final.json", catalogo_final)
    ...
    guardar_resultados(OUT / "reporte.json", reporte)
