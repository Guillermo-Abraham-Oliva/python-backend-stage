...
# DEMOSTRACIÓN RÁPIDA
# =========================
if __name__ == "__main__": # es un interruptor: el bloque debajo solo se ejecuta cuando corres el archivo directamente. Si el archivo se importa desde otro, no se ejecuta.

    # Persona
    personas = [     # aqui se arma una lista con 3 objetos de la clase persona
        Persona("Guille", 40, "Guía espiritual"),     # 1º objeto  se buscaría como Persona[0]
        Persona("Ana", 35, "Desarrolladora Backend"), # 2º objeto  se buscaría como Persona[1]
        Persona("Luis", 29, "Diseñador"),             # 3º objeto  se buscaría como Persona[2]
    ]
    for p in personas: # se recorre la lista ejecutando el metodo (funcion) en cada objeto
        print(p.info())
